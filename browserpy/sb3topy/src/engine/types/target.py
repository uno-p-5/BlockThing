...
class Target:
    ...
    def get_touching(self, util, other):
        ...
        if other == "_mouse_":
            ...
            # New code to handle edge collision (avoid just returning false since _edge_ isn't in the sprite list)
            self.update(util.display, True)
            sprite_rect = self.sprite.rect
            display_rect = util.display.rect

            # Check if the sprite is touching any edge of the display
            touching_left = sprite_rect.left <= display_rect.left
            touching_right = sprite_rect.right >= display_rect.right
            touching_top = sprite_rect.top <= display_rect.top
            touching_bottom = sprite_rect.bottom >= display_rect.bottom
        ...
        if self.shown and other.shown and pg.sprite.collide_mask(other.sprite, self.sprite):
            return True
        ...
        for clone in other.clones:
            ...
            # Check if touching a clone
            if self.shown and clone.shown and pg.sprite.collide_mask(self.sprite, clone.sprite):
                return True
        return False
